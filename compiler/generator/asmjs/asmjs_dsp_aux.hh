...
class EXPORT asmjs_dsp_factory : public dsp_factory, public faust_smartable {
   protected:
      ...
      virtual ~asmjs_dsp_factory() { delete fFactory; }
   public:
...
EXPORT asmjs_dsp_factory* createAsmDSPFactoryFromFile(const string& filename, int argc, const char* argv[],
                                                      string& error_msg);
EXPORT asmjs_dsp_factory* createAsmDSPFactoryFromString(const string& name_app, const string& dsp_content, int argc,
 ...
 *
 * @return a valid asm.js module and additional helper functions as a string on success (to be deleted by the caller),
 ...
 * otherwise a null pointer.
 */
EXPORT const char* createAsmCDSPFactoryFromString(const char* name_app, const char* dsp_content, int argc,
