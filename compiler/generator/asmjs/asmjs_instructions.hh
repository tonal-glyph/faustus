...
public:
    ...
    ASMJAVAScriptInstVisitor(std::ostream* out, int tab = 0) : TextInstVisitor(out, ".", ifloat(), "", tab)
    {
        // Integer version
        ...
        if (array_typed && array_typed->fSize > 1) {
            if (is_struct) {
                ...
                fFieldTable[inst->fAddress->getName()] =
                    make_pair(fStructOffset, Typed::getPtrFromType(array_typed->fType->getType()));
                fStructOffset +=
                    (array_typed->fSize *
    ...
    virtual void visit(DeclareFunInst* inst)
    {
        ...
        if (fMathLibTable.find(inst->fName) != fMathLibTable.end()) {
            if (fMathLibTable[inst->fName] != "manual") {
                tab(fTab, *fOut);
                *fOut << "var " << inst->fName << " = " << fMathLibTable[inst->fName] << ";";
            }
        } else {
            // Prototype
            tab(fTab, *fOut);
            *fOut << "function " << generateFunName(inst->fName);
