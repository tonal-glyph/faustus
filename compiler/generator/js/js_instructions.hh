...
public:
    ...
    JAVAScriptInstVisitor(std::ostream* out, int tab = 0) : TextInstVisitor(out, ".", tab) { initMathTable(); }
    void initMathTable()
    {
    ...
    virtual void visit(AddButtonInst* inst)
    {
        ...
        *fOut << "function handler(obj) { function setval(val) { obj." << inst->fZone
            << " = val; } return setval; }(this))";
        EndLine();
    }
    ...
    virtual void visit(FloatNumInst* inst) { *fOut << inst->fNum; }
    // No . syntax for double in JS
    virtual void visit(DoubleNumInst* inst) { *fOut << inst->fNum; }
