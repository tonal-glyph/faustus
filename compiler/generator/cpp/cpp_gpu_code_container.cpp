...
void CPPOpenCLCodeContainer::produceClass()
{
    ...
    *fGPUOut << "const char* KernelSource = \"";
    // Macro definition
    tab1(n, *fGPUOut);
    *fGPUOut << "#define max(x,y) (((x)>(y)) ? (x) : (y))";
    tab1(n, *fGPUOut);
    ...
    *fGPUOut << "\\n\";";
    ...
    tab(n, *fOut);
    *fOut << dynamic_cast<ostringstream*>(fGPUOut)->str();
    tab(n, *fOut);
    tab(n, *fOut);
    *fOut << "class " << fKlassName << " : public " << fSuperKlassName << " {";
    ...
    if (fDeclarationInstructions->fCode.size() > 0) {
        ...
        *fOut << "} faustcontrol;";
        tab(n + 1, *fOut);
        tab(n + 1, *fOut);
        *fOut << "faustcontrol* fHostControl;";
        tab(n + 1, *fOut);
        *fOut << "cl_mem fDeviceDSP;";
        tab(n + 1, *fOut);
        *fOut << "cl_mem fDeviceControl;";
    }
    tab(n + 1, *fOut);
    *fOut << "cl_device_id fDeviceID;";
    tab(n + 1, *fOut);
    *fOut << "cl_context fContext;";
    tab(n + 1, *fOut);
    *fOut << "cl_command_queue fCommands;";
    tab(n + 1, *fOut);
    *fOut << "cl_program fProgram;";
    tab(n + 1, *fOut);
    *fOut << "cl_kernel fComputeKernel;";
    tab(n + 1, *fOut);
    *fOut << "cl_kernel fInstanceInitKernel;";
    tab(n + 1, *fOut);
    *fOut << "cl_device_id* fDevicesTable;";
    tab(n + 1, *fOut);
    *fOut << "RunThread* fRunThread;";
    tab(n + 1, *fOut);
    *fOut << "int fCount;";
    if (fNumInputs > 0) {
        tab(n + 1, *fOut);
        *fOut << "float** fHostInputs;";
        tab(n + 1, *fOut);
        *fOut << "cl_mem* fDeviceInputs;";
    }
    if (fNumOutputs > 0) {
        tab(n + 1, *fOut);
        *fOut << "float** fHostOutputs;";
        tab(n + 1, *fOut);
        *fOut << "cl_mem* fDeviceOutputs;";
    }
    tab(n + 1, *fOut);
    tab(n, *fOut);
    *fOut << "  public:";
    ...
    produceMetadata(n + 1);
    tab(n + 1, *fOut);
    *fOut << "static double executionTime(cl_event &event) {";
    tab(n + 2, *fOut);
    ...
    *fOut << "}" << endl;
    tab(n + 1, *fOut);
    *fOut << "static void* RunHandler(void* arg) {";
    tab(n + 2, *fOut);
    *fOut << "mydsp* dsp = static_cast<mydsp*>(arg);";
    tab(n + 2, *fOut);
    *fOut << "if (dsp->fRunThread->fRealTime) {";
    tab(n + 3, *fOut);
    ...
    *fOut << "int err = 0;";
    tab(n + 3, *fOut);
    *fOut << "err |= clSetKernelArg(dsp->fComputeKernel, 0, sizeof(int), &dsp->fCount);";
    tab(n + 3, *fOut);
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 4, *fOut);
    *fOut << "std::cerr << \"clSetKernelArg err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "}";
    tab(n + 3, *fOut);
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 4, *fOut);
    *fOut << "std::cerr << \"clGetKernelWorkGroupInfo err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "}";
    tab(n + 3, *fOut);
    *fOut << "cl_event dsp_execution;";
    if (gGlobal->gVectorSwitch) {
        ...
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"clEnqueueNDRangeKernel compute err = \" << err << endl;";
        tab(n + 3, *fOut);
        *fOut << "}";
    } else {
        // Only one kernel
        ...
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"clEnqueueTask compute err = \" << err << endl;";
        tab(n + 3, *fOut);
        *fOut << "}";
    }
    // Wait for computation end
    tab(n + 3, *fOut);
    *fOut << "err = clFinish(dsp->fCommands);";
    tab(n + 3, *fOut);
    *fOut << "if (getenv(\"OCL_GPU_LOAD\") && strtol(getenv(\"OCL_GPU_LOAD\"), NULL, 10)) {";
    tab(n + 4, *fOut);
    ...
    *fOut << "if (gpu_load > val) {";
    tab(n + 5, *fOut);
    *fOut << "cout << \"Execution time = \" <<  gpu_load << \"%\" << endl;";
    tab(n + 4, *fOut);
    *fOut << "}" << endl;
    tab(n + 3, *fOut);
    *fOut << "}" << endl;
    ...
    *fOut << "int err;";
    tab(n + 2, *fOut);
    *fOut << "int gpu = (getenv(\"OCL_GPU\") ? strtol(getenv(\"OCL_GPU\"), NULL, 10) : 0);";
    tab(n + 2, *fOut);
    *fOut << "cl_uint num_devices;";
    tab(n + 2, *fOut);
    *fOut << "char* program_src;";
    if (fNumInputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "fDeviceInputs = new cl_mem[" << fNumInputs << "];";
        tab(n + 2, *fOut);
        *fOut << "fHostInputs = new float*[" << fNumInputs << "];";
    }
    if (fNumOutputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "fDeviceOutputs = new cl_mem[" << fNumOutputs << "];";
        tab(n + 2, *fOut);
        *fOut << "fHostOutputs = new float*[" << fNumOutputs << "];";
    }
    // Creates device
    tab(n + 2, *fOut);
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot count GPU devices err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "if (num_devices == 0) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"There is no GPU devices\"<< endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot get CPU devices err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "fDeviceID = fDevicesTable[(getenv(\"OCL_GPU_DEVICE\") ? strtol(getenv(\"OCL_GPU_DEVICE\"), NULL, 10) : "
    ...
    // Print device name
    tab(n + 2, *fOut);
    *fOut << "char cDevName[1024];";
    tab(n + 2, *fOut);
    *fOut << "err = clGetDeviceInfo(fDeviceID, CL_DEVICE_NAME, sizeof(cDevName), &cDevName, NULL);";
    tab(n + 2, *fOut);
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot get CPU devices err = \" << err << endl;";
    ...
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "std::cerr << \"Device name: \" << cDevName << endl;";
    // Creates context
    tab(n + 2, *fOut);
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot create context err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot create command queue err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot create program err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    tab(n + 2, *fOut);
    *fOut << "err = clBuildProgram(fProgram, 0, NULL, \"-cl-denorms-are-zero\", NULL, NULL);";
    tab(n + 2, *fOut);
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot build program err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error_build;";
    tab(n + 2, *fOut);
    *fOut << "}";
    // Create the compute kernel
    tab(n + 2, *fOut);
    *fOut << "fComputeKernel = clCreateKernel(fProgram, \"computeKernel\", &err);";
    tab(n + 2, *fOut);
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot create compute kernel err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    tab(n + 2, *fOut);
    *fOut << "fInstanceInitKernel = clCreateKernel(fProgram, \"instanceInitKernel\", &err);";
    tab(n + 2, *fOut);
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot create instanceInit kernel err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "} else {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"CL_KERNEL_WORK_GROUP_SIZE = \" << err << local << endl;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    ...
    *fOut << "} else {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"CL_KERNEL_LOCAL_MEM_SIZE = \" << err << local_mem << endl;";
    tab(n + 2, *fOut);
    *fOut << "}";
    // Allocate kernel input buffers (shared between CPU and GPU)
    if (fNumInputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        ...
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot allocate input buffer err = \" << err << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
    ...
    if (fNumOutputs > 0) {
        ...
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot allocate output buffer err = \" << err << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
    ...
    tab(n + 2, *fOut);
    *fOut << "fHostControl = (faustcontrol*)calloc(1, sizeof(faustcontrol));";
    tab(n + 2, *fOut);
    *fOut << "if (fHostControl == NULL) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot allocate control err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot map control err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot allocate DSP err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "err = 0;" << endl;
    if (fNumInputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << "err |= clSetKernelArg(fComputeKernel, i+1, sizeof(cl_mem), &fDeviceInputs[i]);";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    if (fNumOutputs > 0) {
    ...
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "fRunThread = new RunThread();";
    tab(n + 2, *fOut);
    *fOut << "err = fRunThread->Start(true, RunHandler, this);";
    tab(n + 2, *fOut);
    *fOut << "if (err != 0) {";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    ...
    *fOut << "if (err != CL_SUCCESS) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"clGetProgramBuildInfo CL_PROGRAM_BUILD_LOG err = \" << err << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "build_log1[ret_val_size] = '\\0';";
    tab(n + 2, *fOut);
    *fOut << "std::cerr << \"BUILD LOG\" << std::endl;";
    tab(n + 2, *fOut);
    *fOut << "std::cerr << build_log1 << std::endl;";
    tab(n + 1, *fOut);
    *fOut << "error:";
    tab(n + 2, *fOut);
    *fOut << "throw std::bad_alloc();";
    tab(n + 1, *fOut);
    *fOut << "}" << endl;
    tab(n + 1, *fOut);
    *fOut << "virtual ~" << fKlassName << "() {";
    tab(n + 2, *fOut);
    *fOut << "fRunThread->Stop();";
    tab(n + 2, *fOut);
    *fOut << "delete fRunThread;";
    // Free kernel input buffers
    if (fNumInputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << "clReleaseMemObject(fDeviceInputs[i]);";
        tab(n + 3, *fOut);
        *fOut << "delete[] fHostInputs[i];";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    ...
    if (fNumOutputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumOutputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << "clReleaseMemObject(fDeviceOutputs[i]);";
        tab(n + 3, *fOut);
        *fOut << "delete[] fHostOutputs[i];";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    if (fNumInputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "delete[] fDeviceInputs;";
        tab(n + 2, *fOut);
        *fOut << "delete[] fHostInputs;";
    }
    if (fNumOutputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "delete[] fDeviceOutputs;";
        tab(n + 2, *fOut);
        *fOut << "delete[] fHostOutputs;";
    }
    // Shutdown and cleanup
    tab(n + 2, *fOut);
    *fOut << "free(fHostControl);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseMemObject(fDeviceControl);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseMemObject(fDeviceDSP);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseKernel(fComputeKernel);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseKernel(fInstanceInitKernel);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseCommandQueue(fCommands);";
    tab(n + 2, *fOut);
    *fOut << "clReleaseContext(fContext);";
    tab(n + 2, *fOut);
    *fOut << "destroy();";
    tab(n + 1, *fOut);
    *fOut << "}" << endl;
    ...
    *fOut << "virtual void instanceInit(int samplingFreq) {";
    if (fInitInstructions->fCode.size() > 0) {
        ...
        *fOut << "int err = 0;";
        tab(n + 2, *fOut);
        *fOut << "err |= clSetKernelArg(fInstanceInitKernel, 0, sizeof(cl_mem), &fDeviceDSP);";
        tab(n + 2, *fOut);
        ...
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 3, *fOut);
        *fOut << "std::cerr << \"clSetKernelArg instanceInit err = \" << err << endl;";
        tab(n + 2, *fOut);
        *fOut << "}";
        tab(n + 2, *fOut);
        *fOut << "err = clEnqueueTask(fCommands, fInstanceInitKernel, 0, NULL, NULL);";
        tab(n + 2, *fOut);
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 3, *fOut);
        *fOut << "std::cerr << \"clEnqueueTask instanceInit err = \" << err << endl;";
        tab(n + 2, *fOut);
        *fOut << "}";
        // Wait for instanceInit end
        tab(n + 2, *fOut);
        *fOut << "err = clFinish(fCommands);";
        tab(n + 2, *fOut);
        *fOut << "if (err != CL_SUCCESS) {";
        tab(n + 3, *fOut);
        *fOut << "std::cerr << \"clFinish instanceInit err = \" << err << endl;";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    tab(n + 1, *fOut);
...
void CPPOpenCLCodeContainer::generateCompute(int n)
{
    ...
    *fOut << "getRealTime();";
    ...
    if (fNumInputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << subst("memcpy(fHostInputs[i], inputs[i], sizeof($0) * count);", xfloat());
        tab(n + 2, *fOut);
        *fOut << "}";
        tab(n + 2, *fOut);
    }
    ...
    if (fNumOutputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumOutputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << subst("memcpy(outputs[i], fHostOutputs[i], sizeof($0) * count);", xfloat());
        tab(n + 2, *fOut);
        *fOut << "}";
        tab(n + 2, *fOut);
    }
    tab(n + 2, *fOut);
    *fOut << "fRunThread->Signal();";
    tab(n + 1, *fOut);
    *fOut << "}";
}
...
void CPPCUDACodeContainer::generateComputeKernelGlue(int n)
{
    ...
    *fGPUOut << "dim3 grid(1);";
    tab(n + 1, *fGPUOut);
    *fGPUOut << "computeKernel<<<grid, block>>>(count, ";
    for (int i = 0; i < fNumInputs; i++) {
        *fGPUOut << "input" << i << ", ";
    }
    for (int i = 0; i < fNumOutputs; i++) {
...
void CPPCUDACodeContainer::produceClass()
{
    ...
    *fOut << "class " << fKlassName << " : public " << fSuperKlassName << " {";
    ...
    if (fDeclarationInstructions->fCode.size() > 0) {
        ...
        tab(n + 1, *fOut);
        tab(n + 1, *fOut);
        *fOut << "faustdsp* fDeviceDSP;";
        tab(n + 1, *fOut);
        *fOut << "faustcontrol* fHostControl;";
        tab(n + 1, *fOut);
        *fOut << "faustcontrol* fDeviceControl;";
    }
    tab(n + 1, *fOut);
    *fOut << "int fDeviceID;";
    tab(n + 1, *fOut);
    *fOut << "volatile bool fDeviceRunning;";
    tab(n + 1, *fOut);
    *fOut << "int fDeviceCount;";
    tab(n + 1, *fOut);
    *fOut << "RunThread* fRunThread;";
    tab(n + 1, *fOut);
    *fOut << "int fCount;";
    if (fNumInputs > 0) {
        tab(n + 1, *fOut);
        *fOut << "float** fDeviceInputs;";
        tab(n + 1, *fOut);
        *fOut << "float** fHostInputs;";
    }
    if (fNumOutputs > 0) {
        tab(n + 1, *fOut);
        *fOut << "float** fDeviceOutputs;";
        tab(n + 1, *fOut);
        *fOut << "float** fHostOutputs;";
    }
    tab(n, *fOut);
    ...
    *fOut << "mydsp* dsp = static_cast<mydsp*>(arg);";
    tab(n + 2, *fOut);
    *fOut << "cudaError_t cudaResult;";
    tab(n + 2, *fOut);
    *fOut << "cudaResult = cudaSetDeviceFlags(cudaDeviceMapHost);";
    tab(n + 2, *fOut);
    ...
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "while (dsp->fDeviceRunning) {";
    tab(n + 3, *fOut);
    *fOut << "computeKernelGlue(dsp->fCount, ";
    if (fNumInputs > 0) {
        for (int i = 0; i < fNumInputs; i++) {
    ...
    *fOut << "cudaError_t cudaResult;";
    if (fNumInputs > 0) {
        ...
        *fOut << "fHostInputs = new float*[" << fNumInputs << "];";
        tab(n + 2, *fOut);
        *fOut << "fDeviceInputs = new float*[" << fNumInputs << "];";
    }
    if (fNumOutputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "fHostOutputs = new float*[" << fNumOutputs << "];";
        tab(n + 2, *fOut);
        *fOut << "fDeviceOutputs = new float*[" << fNumOutputs << "];";
    }
    tab(n + 2, *fOut);
    *fOut << "fHostControl = NULL;";
    // Creates device
    tab(n + 2, *fOut);
    *fOut << "cudaResult = cudaGetDeviceCount(&fDeviceCount);";
    tab(n + 2, *fOut);
    *fOut << "if (cudaResult != cudaSuccess) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot count GPU devices err = \" << cudaResult << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "if (fDeviceCount == 0) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"There is no GPU devices\"<< endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    ...
    *fOut << "fRunThread = new RunThread();";
    tab(n + 2, *fOut);
    *fOut << "if (fRunThread->Start(true, RunHandler, this) != 0) {";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "while(!fDeviceRunning) { usleep(100); }";
    tab(n + 2, *fOut);
    *fOut << "return;" << endl;
    tab(n + 1, *fOut);
    *fOut << "error:";
    ...
    *fOut << "virtual ~" << fKlassName << "() {";
    tab(n + 2, *fOut);
    *fOut << "fDeviceRunning = false; ";
    // tab(n+2, *fOut); *fOut << "fRunThread->Stop();";
    tab(n + 2, *fOut);
    *fOut << "fRunThread->Join();";
    ...
    *fOut << "}" << endl;
    tab(n + 1, *fOut);
    *fOut << "bool allocateCUDA() {";
    tab(n + 2, *fOut);
    *fOut << "cudaError_t cudaResult;";
    tab(n + 2, *fOut);
    *fOut << "fDeviceID = 1;";
    tab(n + 2, *fOut);
    *fOut << "cudaResult = cudaSetDevice(fDeviceID);";
    tab(n + 2, *fOut);
    *fOut << "if (cudaResult != cudaSuccess) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot set GPU device err = \" << cudaResult << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    // Print device name
    tab(n + 2, *fOut);
    *fOut << "cudaDeviceProp deviceProp;";
    tab(n + 2, *fOut);
    *fOut << "cudaResult = cudaGetDeviceProperties(&deviceProp, fDeviceID);";
    tab(n + 2, *fOut);
    *fOut << "if (cudaResult != cudaSuccess) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot get device properties err = \" << cudaResult << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "std::cerr << \"Device name: \" << deviceProp.name << endl;";
    tab(n + 2, *fOut);
    *fOut << "if (!deviceProp.canMapHostMemory) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Device cannot map host memory \" << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "}";
    ...
    if (fNumInputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << subst(
            "cudaResult = cudaHostAlloc((void **)&fHostInputs[i], sizeof($0) * 8192, "
            "cudaHostAllocMapped|cudaHostAllocPortable);",
            xfloat());
        tab(n + 3, *fOut);
        *fOut << "if (cudaResult != cudaSuccess) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot allocate input buffer err = \" << cudaResult << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
        ...
        *fOut << "if (cudaResult != cudaSuccess) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot map input buffer err = \" << cudaResult << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
    ...
    if (fNumOutputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumOutputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << subst(
            "cudaResult = cudaHostAlloc((void **)&fHostOutputs[i], sizeof($0) * 8192, "
            "cudaHostAllocMapped|cudaHostAllocPortable);",
            xfloat());
        tab(n + 3, *fOut);
        *fOut << "if (cudaResult != cudaSuccess) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot allocate output buffer err = \" << cudaResult << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
        ...
        *fOut << "if (cudaResult != cudaSuccess) {";
        tab(n + 4, *fOut);
        *fOut << "std::cerr << \"Cannot map output buffer err = \" << cudaResult << endl;";
        tab(n + 4, *fOut);
        *fOut << "goto error;";
        tab(n + 3, *fOut);
        *fOut << "}";
    ...
    *fOut << "if (cudaResult != cudaSuccess) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot allocate control err = \" << cudaResult << endl;";
    tab(n + 3, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    ...
    *fOut << "if (cudaResult != cudaSuccess) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot map control err = \" << cudaResult << endl;";
    tab(n + 4, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    // Allocate DSP on GPU
    tab(n + 2, *fOut);
    *fOut << "cudaResult = cudaMalloc((void **)&fDeviceDSP, sizeof(faustdsp));";
    tab(n + 2, *fOut);
    *fOut << "if (cudaResult != cudaResult) {";
    tab(n + 3, *fOut);
    *fOut << "std::cerr << \"Cannot allocate DSP err = \" << cudaResult << endl;";
    tab(n + 4, *fOut);
    *fOut << "goto error;";
    tab(n + 2, *fOut);
    *fOut << "}";
    tab(n + 2, *fOut);
    *fOut << "fDeviceRunning = true;";
    tab(n + 2, *fOut);
    *fOut << "return true;";
    tab(n + 1, *fOut);
    *fOut << "error:";
    ...
    *fOut << "}";
    tab(n + 1, *fOut);
    *fOut << "void destroyCUDA() {";
    // Free kernel input buffers
    if (fNumInputs > 0) {
        tab(n + 2, *fOut);
        *fOut << "for (int i = 0; i < " << fNumInputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << "cudaFreeHost(fHostInputs[i]);";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    // Free kernel output buffers
    if (fNumOutputs > 0) {
        ...
        *fOut << "for (int i = 0; i < " << fNumOutputs << "; i++) {";
        tab(n + 3, *fOut);
        *fOut << "cudaFreeHost(fHostOutputs[i]);";
        tab(n + 2, *fOut);
        *fOut << "}";
    }
    if (fNumInputs > 0) {
    ...
    tab(n + 2, *fOut);
    *fOut << "cudaFreeHost(fDeviceControl);";
    tab(n + 2, *fOut);
    *fOut << "cudaFree(fDeviceDSP);";
    // Shutdown and cleanup
    tab(n + 2, *fOut);
    *fOut << "cudaThreadExit();";
    tab(n + 1, *fOut);
