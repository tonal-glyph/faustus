...
LLVMContext& LLVMCodeContainer::getContext()
{
    return *fContext;
}
...
void LLVMCodeContainer::generateFillBegin(const string& counter)
{
    ...
    FunctionType* llvm_fill_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_fill_args), false);
...
void LLVMCodeContainer::generateGetSampleRate(int field_index)
{
    ...
    FunctionType* llvm_getSR_type =
        FunctionType::get(fBuilder->getInt32Ty(), MAKE_VECTOR_OF_TYPES(llvm_getSR_args), false);
    Function* sr_fun =
        Function::Create(llvm_getSR_type, Function::ExternalLinkage, "getSampleRate" + fKlassName, fModule);
...
void LLVMCodeContainer::generateClassInitBegin()
{
    ...
    FunctionType* llvm_classInit_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_classInit_args), false);
    Function* llvm_classInit =
        Function::Create(llvm_classInit_type, Function::ExternalLinkage, "classInit" + fKlassName, fModule);
...
void LLVMCodeContainer::generateInstanceInitBegin(const string& funname, bool internal)
{
    ...
    FunctionType* llvm_instanceInit_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_instanceInit_args), false);
    Function* llvm_instanceInit =
        Function::Create(llvm_instanceInit_type, (internal) ? Function::InternalLinkage : Function::ExternalLinkage,
                        funname + fKlassName, fModule);
...
void LLVMCodeContainer::generateInstanceClearBegin(bool internal)
{
    ...
    FunctionType* llvm_instanceClear_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_instanceClear_args), false);
    Function* llvm_instanceClear =
        Function::Create(llvm_instanceClear_type, (internal) ? Function::InternalLinkage : Function::ExternalLinkage,
                        "instanceClear" + fKlassName, fModule);
...
void LLVMCodeContainer::generateInstanceResetUserInterfaceBegin(bool internal)
{
    ...
    FunctionType* llvm_instanceResetUserInterface_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_instanceResetUserInterface_args), false);
    Function* llvm_instanceResetUserInterface = Function::Create(
        llvm_instanceResetUserInterface_type, (internal) ? Function::InternalLinkage : Function::ExternalLinkage,
...
void LLVMCodeContainer::generateInitFun()
{
    ...
    FunctionType* llvm_init_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_init_args), false);
...
void LLVMCodeContainer::generateInstanceInitFun()
{
    ...
    FunctionType* llvm_init_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_init_args), false);
    Function* llvm_init =
        Function::Create(llvm_init_type, Function::ExternalLinkage, "instanceInit" + fKlassName, fModule);
    llvm_init->setCallingConv(CallingConv::C);
    Function::arg_iterator llvm_init_args_it = llvm_init->arg_begin();
    Value*                 arg1              = GET_ITERATOR(llvm_init_args_it++);
    arg1->setName("dsp");
...
void LLVMCodeContainer::generateMetadata(llvm::PointerType* meta_type_ptr)
{
    ...
    FunctionType* llvm_metaData_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_metaData_args), false);
    Function* llvm_metaData =
        Function::Create(llvm_metaData_type, GlobalValue::ExternalLinkage, "metadata" + fKlassName, fModule);
...
void LLVMCodeContainer::produceInternal()
{
    ...
    fCodeProducer = new LLVMInstVisitor(fModule, fBuilder, fAllocaBuilder, fTypeBuilder.getFieldNames(),
                                        fTypeBuilder.getUIPtr(), fStructDSP, fKlassName);
    generateFunMaps();
...
void LLVMWorkStealingCodeContainer::generateComputeThreadBegin()
{
    ...
    FunctionType* llvm_computethread_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_computethread_args), false);
    Function* llvm_computethread =
        Function::Create(llvm_computethread_type, GlobalValue::ExternalLinkage, "computeThread", fModule);
...
void LLVMWorkStealingCodeContainer::generateComputeThreadExternal()
{
    ...
    FunctionType* llvm_computethread_type =
        FunctionType::get(fBuilder->getVoidTy(), MAKE_VECTOR_OF_TYPES(llvm_computethread_args), false);
    Function* llvm_computethread =
        Function::Create(llvm_computethread_type, GlobalValue::ExternalLinkage, "computeThreadExternal", fModule);
    llvm_computethread->setCallingConv(CallingConv::C);
    Function::arg_iterator llvm_computethread_args_it = llvm_computethread->arg_begin();
    Value*                 arg1                       = GET_ITERATOR(llvm_computethread_args_it++);
    arg1->setName("dsp");
...
SeqLoopBuilderVisitor loop_sequencer(dynamic_cast<ForLoopInst*>(loop->clone(&cloner)),
dynamic_cast<ForLoopInst*>(loop->clone(&cloner))); Loop2FunctionBuider builder1("combined1",
loop_sequencer.fResultLoop); builder1.fFunctionDef->accept(fCodeProducer);
...
StatementInst* fun1 = InstBuilder::genDeclareFunInst("fun1", InstBuilder::genFunTyped(types,
InstBuilder::genBasicTyped(Typed::kVoid)), code); fun1->accept(fCodeProducer);
*/
/*
...
(getNumInputsFun)(intptr_t)fJIT->getPointerToFunction(fModule->getFunction("getNumInputs_llvm")); printf("function
%x\n", fGetNumInputs); llvm_dsp dsp; int num = fGetNumInputs(&dsp); printf("module num %d\n", num);
*/
/*
